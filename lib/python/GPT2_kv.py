...
extended_attention_mask = torch.cat(
    [
        attention_mask,
        torch.ones(
            (attention_mask.size(0), new_input_ids.size(-1)), dtype=attention_mask.dtype
        ),
    ],
    dim=-1,
)
...
past_key_values_zero = torch.zeros(torch.Size([24, 1, 12, 1, 64]))
...
onnx_model = onnx.load("gpt2_with_kv.onnx")
...
for i in range(15):
    print(f"\nIteration {i}:")
    # Calculate the total sequence length (past + current)
    total_sequence_length = past_key_values_ort.shape[3] + next_input_ort.shape[1]

    # Create a fresh attention mask of the correct size
    extended_attention_mask = torch.ones((1, total_sequence_length), dtype=torch.int8)
    extended_attention_mask_ort = extended_attention_mask.numpy()

    # Update inputs with corrected dimensions
    onnx_inputs = {
        "input_ids": next_input_ort,
        "attention_mask": extended_attention_mask_ort,
        "past_key_values_input": past_key_values_ort,
    ...
    token = output_id_ort[0][0]
    print(f"Generated token: {token}")
    print(f"Generated text: {tokenizer.decode([token])}")

    # Prepare next input
    next_input = torch.tensor([[output_id_ort[0][0]]])
    next_input_ort = next_input.numpy()

    # Extend attention mask correctly
    extended_attention_mask = torch.cat(
        [
            extended_attention_mask,
            torch.ones((1, 1), dtype=torch.int8),  # Match the dtype
        ],
        dim=1,
    )
    extended_attention_mask_ort = extended_attention_mask.numpy()
